 ...
 *
 * `*.vercel.app` is included so email/password on a Vercel deploy is not rejected
 * as "Invalid origin". The Grok broker still only allows `*.grok-sandbox.com`
 * callbacks — Google/X on vercel.app will not complete (login UI hides them).
 */
